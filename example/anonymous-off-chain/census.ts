...
export async function generatePublicCensusFromAccounts(accounts: TestAccount[], entityWallet: Wallet, gwPool: IGatewayClient) {
    ...
    const { invalidClaims, censusRoot } = await CensusOffChainApi.addClaimBulk(censusId, claimList, entityWallet, gwPool)
    ...
    const censusSize = await CensusOffChainApi.getSize(censusId, gwPool)
    if (config.stopOnError) {
