...
import { CSP, IGatewayClient } from "@vocdoni/client"
import { ensure0x, hexStringToBuffer, Random } from "@vocdoni/common"
...
export async function submitVotes(hexTokenR: string, processId: string, processParams: ProcessContractParameters, processMetadata: ProcessMetadata, gwPool: IGatewayClient, csp: CSP) {
  ...
  await Bluebird.map(idxs, async (_, idx: number) => {
    ...
    const { hexBlinded: blindedPayload, userSecretData } = CspSignatures.getBlindedPayload(processId, hexTokenR, wallet)

    const blindSignature = await CspSignatures.getSignature("blind", blindedPayload, hexTokenR, processId, csp)
    ...
    const envelope = processParams.envelopeType.hasEncryptedVotes ?
      Voting.packageSignedEnvelope({ censusOrigin: processParams.censusOrigin, votes: choices, censusProof: proof, processId, processKeys }) :
      Voting.packageSignedEnvelope({ censusOrigin: processParams.censusOrigin, votes: choices, censusProof: proof, processId })
