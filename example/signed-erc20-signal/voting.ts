...
import { ProcessState, VochainWaiter, VotingApi, VotingOracleApi, Voting } from "@vocdoni/voting"
import {
    ProcessEnvelopeType,
    ProcessMode,
...
export async function launchNewVote(creatorWallet: Wallet, gwPool: IGatewayClient) {
    ...
    const proof = await CensusErc20Api.generateProof(config.tokenAddress, creatorWallet.address, tokenInfo.balanceMappingPosition, sourceBlockHeight, gwPool.provider as JsonRpcProvider)
