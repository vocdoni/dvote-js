...
export class ContentHashedUri extends ContentUri {
  ...
  constructor(contentHashedUri: string) {
    if (typeof contentHashedUri != "string") {
      throw new Error("Invalid contentHashedUri");
    }
    ...
    if (terms[1]) {
      if (!/^(0x)?[0-9a-fA-F]+$/.test(terms[1])) {
        throw new Error("Invalid hash hex string");
      } else if (terms[1].length % 2 != 0) {
        throw new Error("The hash contains an odd length");
      }
  ...
  /** Returns a ContentHashedUri instance from the various supported input types */
  static override resolve(
    input: string | ContentUri | ContentHashedUri,
  ): ContentHashedUri {
    if (typeof input == "string") return new ContentHashedUri(input);
    else if (input instanceof ContentHashedUri) return input;

    return new ContentHashedUri(input.toString());
  }
}
