export class ContentUri {
  ...
  /** Returns a ContentUri instance from the various supported input types */
  static resolve(input: string | ContentUri): ContentUri {
    if (input instanceof ContentUri) return input;
    return new ContentUri(input);
  }
}
