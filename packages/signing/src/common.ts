 ...
 */
export function signRaw(request: Uint8Array, walletOrSigner: Wallet | Signer): Promise<string> {
 ...
 */
export function isValidRaw(signature: string, publicKey: string, messageBytes: Uint8Array): boolean {
 ...
 *
 * @param signature Hex encoded signature (created with the Ethereum prefix)
 * @param responseBody Buffer being signed
 * @param expanded Whether the resulting public key should be expanded or not (default: no)
 */
export function recoverPublicKeyRaw(messageBytes: Uint8Array, signature: string, expanded: boolean = false): string {
...
export function digestVocdoniTransaction(payload: string | Uint8Array, chainId: string): Uint8Array {
  const prefix = "Vocdoni signed transaction:\n" + chainId + "\n"

  return digestVocdoniPayload(payload, prefix)
}

function digestVocdoniPayload(payload: string | Uint8Array, prefix: string): Uint8Array {
