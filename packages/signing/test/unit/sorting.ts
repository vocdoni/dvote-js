...
describe("JSON normalized strings", () => {
  it("Should reorder JSON objects alphabetically", () => {
    ...
    strA = JsonSignature.normalizedJsonString({ a: 1, b: [{ a: 10, m: 10, z: 10 }, { b: 11, n: 11, y: 11 }, 4, 5] })
    strB = JsonSignature.normalizedJsonString({ b: [{ z: 10, m: 10, a: 10 }, { y: 11, n: 11, b: 11 }, 4, 5], a: 1 })
    expect(strA).to.equal(strB)
