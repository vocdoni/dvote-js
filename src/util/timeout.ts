 ...
 */
export function promiseFuncWithTimeout<T>(func: () => Promise<T>, timeout: number, timeoutMessage?: string): Promise<T> {
...
export function promiseWithTimeout<T>(prom: Promise<T>, timeout: number, timeoutMessage?: string): Promise<T> {
    if (!prom || typeof prom.then !== "function" || typeof prom.catch != "function") throw new Error("Invalid promise")
    else if (isNaN(timeout) || timeout < 0) throw new Error("Invalid timeout")
