...
import { GatewayMock, InteractionMock } from "../mocks/gateway"
    ...
    describe("Swarm", () => {
        it("Should upload a file", async () => {
            ...
            const interactionList: InteractionMock[] = [
                { responseData: { error: false, response: ["bzz://1234"] } }
            ]
        ...
        it("Should retrieve a pinned file", async () => {
            ...
            // Gateway (server)
            const interactionList: InteractionMock[] = [
                { responseData: { error: false, response: ["bzz://2345"] } },
                { responseData: { error: false, response: [base64File] } },
            ]
            const gatewayServer = new GatewayMock({ port, interactionList })
