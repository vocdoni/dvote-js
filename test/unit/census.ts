...
describe("Census", () => {
    ...
    describe("Poseidon hashing", () => {
        it("Should hash public keys properly", () => {
            // 1
            let wallet = Wallet.fromMnemonic("fly cheap color olive setup rigid april forum over grief predict pipe toddler argue give")
            ...
            // 2
            wallet = Wallet.fromMnemonic("kangaroo improve enroll almost since stock travel grace improve welcome orbit decorate govern hospital select")
            ...
            // 3
            wallet = Wallet.fromMnemonic("soup sunset inhale lend eagle hold reduce churn alpha torch leopard phrase unfold crucial soccer")
            pubKey = wallet["signingKey"].publicKey  // 0x04...
        ...
        it("Should fail on invalid hex strings", () => {
            let wallet = Wallet.fromMnemonic("soul frequent purity regret noble husband weapon scheme cement lamp put regular envelope physical entire")
